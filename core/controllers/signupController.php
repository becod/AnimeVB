...
if (isset($_POST['name'])&&isset($_POST['lname'])&&isset($_POST['mail'])&&isset($_POST['user'])&&isset($_POST['pass'])){
    ...
    $query -> queryList('INSERT INTO avb_users(user_name,user_lastname,user_login,user_password,user_email) VALUES (:name, :lastname, :user, :password, :email)', array(':name' => $name, ':lastname' => $lname, ':user' => $username, ':password' => sha1($password), ':email' => $mail));
    if ($query == true ){
            session_start();
            $_SESSION['name']= $name;
